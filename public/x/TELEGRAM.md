Don't roll thirteen.

Coup Ahoo is a dice hull on Robinhood Chain. You play it now. $AHOO launches after The Hold is on-chain. This is the loop and the pipes.

Play: https://coupahoo.lol
X: https://x.com/CoupAhoo
Telegram: https://t.me/CoupAhoo
GitHub: https://github.com/CoupAhoo/coupahoo

—— MECHANICS ——

You roll 3d6. That stack is your ship.

Pips = hit points.
Dice count = guns. More crates, more shots.
Empty deck = you sink.

If the pips sum to 13 the hull is cursed. The first ball of every volley goes overboard. Thirteen is not a score. It is a hole.

Combat dice are 0 / 0 / 1 / 1 / 2 / 2. Roll a volley, keep or reroll, then click their cargo.

Between fights: merchants, shipwrights, sailors in dinghies, storms, free crates in the drink. Gold hires crew.

Gunner — first shot each volley +1.
Carpenter — after a win, patches the weakest cargo +1.
Lookout — enemy wastes their first shot.
Cook — +3 gold every ship you sink.

Plate a crate and it only takes 1 damage. Take the fleet one sloop at a time. Nine encounters. Last one is the flagship.

Play: https://coupahoo.lol/play
Lore: https://coupahoo.lol/lore

—— INFRASTRUCTURE ——

$AHOO · LetsCash · Robinhood Chain (4663) · quote ETH · LP locked at launch.

Tax: 3% of the ETH leg.
Platform 0.3%.
2.7% to The Hold.

The Hold split:

70% prize — best logged run of each 15-minute epoch. Anyone may settle. No keeper. 80% of that pot pays. 1% of the payout is a closer tip. Rest rolls. Empty epochs emit and move on.

20% drip — accrues on the contract. Cabin may send it later to a distributor. Not a claim button. If we have not sent it, it is still sitting in The Hold.

10% cabin — studio. Withdraw only from cabin.

Hold: https://coupahoo.lol/hold
Token: https://coupahoo.lol/token

—— ORDER ——

Play is live.
Hold deploys next (Remix, cabin in the constructor, verify on Blockscout).
Token is last. Fee recipient = Hold CA. Tax 3%.

CA posts when LetsCash is live.

Ship & dice loop inspired by Coup Ahoo © Antti Haavikko (js13k). Hold, token and chain original. Sprites and song not ported.
